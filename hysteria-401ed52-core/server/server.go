...
func NewH3sHandler(config *Config, conn quic.Connection) *H3sHandler {
	return &H3sHandler{
		...
		Conn:             conn,
		ConnID:           rand.Uint32(),
		AuthFinishedChan: make(chan struct{}),
	}
}
...
func (h *H3sHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	if r.Method == http.MethodPost && r.Host == protocol.URLHost && r.URL.Path == protocol.URLPath {
		...
		if ok {
			...
			h.AuthFinishedMutex.Lock()
			if !h.AuthFinished {
				h.AuthFinished = true
				h.AuthFinishedChan <- struct{}{}
				close(h.AuthFinishedChan)
			}
			h.AuthFinishedMutex.Unlock()
		} else {
